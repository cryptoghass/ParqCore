...
*Setup instructions for a gitian build of Parq using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Parq
...
Clone the git repositories for Parq and gitian and then checkout the Parq version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/parqcoin/parq-core.git
cd parq-core
...
Follow the instructions in [doc/release-process.md](release-process.md) in the parq-core repository
...
To build Parq (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the parq-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/parq-core/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/parqcoin/parq-core
...
./bin/gbuild --commit parq=${COMMIT} --url parq=${URL} ../parq/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit parq=${COMMIT} --url parq=${URL} ../parq/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit parq=${COMMIT} --url parq=${URL} ../parq/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/parq-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/parq-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/parq-build.assert
```
...
[parqcoin/gitian.sigs](https://github.com/parqcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
