...
#if defined(NDEBUG)
#error "Parq cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Parq Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) return 90000 * COIN;
	...
	if(nHeight > 0 && nHeight <= 500) {
		...
		nSubsidy = 1.75 * COIN;
	} else if (nHeight > 115000 && nHeight <= 130000) {
		nSubsidy = 0.75 * COIN;
	} else if (nHeight > 130000 && nHeight <= 200000) {
		nSubsidy = 0.5 * COIN;
	} else if (nHeight > 200000 && nHeight <= 2500000) {
		nSubsidy = 0.3 * COIN;
	} else if (nHeight > 2500000 && nHeight <= 9000000) {
		nSubsidy = 0.2 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	int64_t ret = 0;
	// 85% for Masternodes
	if(nHeight > 0 && nHeight <= 502) {
        ret = blockValue / 100 * 0;
	} else if (nHeight > 502 && nHeight <= 9000000) {
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[LIBRA] Adjusting Libra at height %d with %d masternodes (%d % locked Parq) and %d Parq supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
