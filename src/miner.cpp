...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ParqMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ParqMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ParqMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("parq-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ParqMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ParqMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadParqMiner() exception");
    } catch (...) {
        LogPrintf("ThreadParqMiner() exception");
    }

    LogPrintf("ThreadParqMiner exiting\n");
}
